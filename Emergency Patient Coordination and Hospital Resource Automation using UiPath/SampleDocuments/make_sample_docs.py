...
How the console uses them (for reference):
  ...
  * VerifyDocuments only checks the file EXISTS - it does not parse the contents.
  * So the body of these files can say anything; the slot you upload into and a
    valid extension (.pdf/.png/.jpg/.jpeg) are what matter.
  * The 'Insurance' slot is the one that also drives a later stage: at discharge,
    ProcessDischarge builds an insurance claim packet only if Insurance is Present.
...
def medical_report(p):
    return [
        ...
        ("h2", "PATIENT"),
        ("kv", "Name .................. %s" % p["name"]),
        ("kv", "Age / Gender ......... %s / %s" % (p["age"], p["gender"])),
        ("kv", "Reference ............ %s" % p.get("ref", "(assigned at registration)")),
        ("kv", "Date / Time .......... %s" % p["now"]),
        ("gap", ""),
        ("h2", "CLINICAL SUMMARY (referring team)"),
        ...
        ("kv", "Blood group ......... %s   Units anticipated: %s" % (p["blood"], p["units"])),
        ...
        ("kv", "Signature ........... ____________________________"),
...
def insurance(p):
    return [
        ...
        ("kv", "Insurer helpline .. 1800-000-000 (test)"),
        ("gap", ""),
...
def consent_form(p):
    return [
        ...
        ("gap", ""),
        ("kv", "Patient ............ %s" % p["name"]),
        ("kv", "Consent given by ... %s (%s)" % (p["consenter"], p["relationship"])),
        ("kv", "Date / Time ....... %s" % p["now"]),
        ("kv", "Signature ......... ____________________________"),
...
PATIENTS = [
    dict(pack="patient_A_rahul", name="Rahul Kumar", age="54", gender="Male",
         ...
         id_no="TEST-1122-3344-5566", address="14 MG Road, Bengaluru 560001",
    ...
    dict(pack="patient_B_ananya", name="Ananya Singh", age="32", gender="Female",
         ...
         id_no="TEST-7788-9900-1122", address="221 Park Street, Kolkata 700016",
    ...
    dict(pack="patient_C_minor", name="Meera R", age="16", gender="Female",
         ...
         id_no="TEST-3344-5566-7788", address="7 Lake View, Pune 411001",
    ...
    for base in PATIENTS:
        ...
        with open(os.path.join(outdir, "index.txt"), "w", encoding="utf-8") as f:
            f.write("\n".join([
                ...
                "",
                "Note: the bot checks the file EXISTS; it does not read the contents.",
            ]) + "\n")
